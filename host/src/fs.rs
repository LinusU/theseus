    ...
    fn normalize(path: &Path) -> String {
        ...
        if !path.starts_with('/') {
            ...
            for part in cwd.split('/') {
                if !part.is_empty() {
                    // Leaked to keep the borrow simple; paths are short-lived
                    // and few, and this only runs in the wasm build.
                    parts.push(Box::leak(part.to_string().into_boxed_str()));
                }
            }
        }
        for part in path.split('/') {
            match part {
                ...
                part => parts.push(Box::leak(part.to_string().into_boxed_str())),
            }
        }
        format!("/{}", parts.join("/"))
    }
