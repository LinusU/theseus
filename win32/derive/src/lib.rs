...
pub fn dllexport(attr: TokenStream, mut tokens: TokenStream) -> TokenStream {
    ...
    let fetch_args = {
        ...
        for (i, (arg, ty)) in args.iter().enumerate() {
            ...
            v.push(quote! {
                let #arg = <#ty>::from_abi(ctx.memory.read::<u32>(ctx.cpu.regs.esp + #offset));
            });
        }
        quote!(#(#v)*)
    };
    ...
    let trace = {
        ...
        quote![log::info!(#fmt_string, return_addr = if return_addr == RETURN_FROM_X86_ADDR32 { ctx.cpu.regs.eip_context } else { return_addr });]
    };
