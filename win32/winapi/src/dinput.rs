...
use crate::{ddraw::GUID, heap::Heap, kernel32, user32};
...
pub struct State {
    ...
    pub devices: Mutex<HashMap<u32, Device>>,
}
...
pub mod IDirectInputDevice {
    ...
    fn set_acquired(this: u32, acquired: bool) {
        if let Some(device) = state().devices.lock().unwrap().get_mut(&this) {
    ...
    pub fn Release(_ctx: &mut Context, this: u32) -> u32 {
        state().devices.lock().unwrap().remove(&this);
        0
    }
    ...
    pub fn GetDeviceData(
        ...
        // A null array means the caller only wants the pending event count.
        let capacity = if rgdod == 0 {
            usize::MAX
        } else {
            ctx.memory.read::<u32>(pdwInOut) as usize
        };
        let peek = rgdod == 0 || dwFlags & DIGDD_PEEK != 0;
