...
fills in before starting it. Point `dataRoot` in host.ts at a directory holding
the files plus a `manifest.json` listing them, and set `cwd` to the directory
the program should start in. Files the program writes are kept in localStorage
and restored on the next load.

URL parameters: `?frames=1` reports progress and window contents to the server,
`?trace=<spec>` traces winapi calls in the same syntax as `THESEUS_TRACE`.
