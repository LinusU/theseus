...
impl FPU {
    ...
    pub fn set_cmp(&mut self, cmp: impl Into<Option<std::cmp::Ordering>>) {
        self.cc = match cmp.into() {
...
mod tests {
    ...
    fn compare_condition_codes() {
        ...
        fpu.set_cmp(1.0f64.partial_cmp(&2.0));
        assert_eq!(condition_codes(&fpu), Status::C0.bits());
        fpu.set_cmp(2.0f64.partial_cmp(&2.0));
        assert_eq!(condition_codes(&fpu), Status::C3.bits());
        fpu.set_cmp(2.0f64.partial_cmp(&1.0));
        assert_eq!(condition_codes(&fpu), 0);
        fpu.set_cmp(f64::NAN.partial_cmp(&1.0));
        assert_eq!(
            condition_codes(&fpu),
    ...
    fn unordered_reaches_parity_flag() {
        ...
        let mut fpu = FPU::default();
        fpu.set_cmp(1.0f64.partial_cmp(&1.0));
