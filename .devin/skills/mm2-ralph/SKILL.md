...
Run the relevant formatter, check, test, or target build. Append a concise entry to `doc/mm2-progress.md` containing the command, result, and next blocker. If the working tree was clean at the start and the iteration succeeds, stage only this iteration's tracked changes and create one commit before stopping. Use a concise why-focused message with the standard Devin footer; never include `game/` or generated artifacts. If the iteration fails or is blocked, leave its changes uncommitted for review. Do not push, reset, or discard existing user changes.
