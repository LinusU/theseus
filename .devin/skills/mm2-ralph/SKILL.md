...
At the start of each milestone, reread `doc/mm2-progress.md`, choose the next smallest actionable blocker, reproduce it, make the smallest correct implementation change, and add or update a focused test when the code has test infrastructure. Run `.devin/check.sh` (it formats, lints, tests, and builds what changed). A failing build or test is diagnostic information, not a reason to stop. Record the milestone in `doc/mm2-progress.md` with that file's entry template, about six lines; grep `doc/mm2-progress-archive.md` for history instead of reading it. Baseline, verification-sweep, and close-out entries are not milestones and are not committed on their own.
