...
impl<'a> CodeGen<'a> {
    fn fpu_get_mem(&self, instr: &iced_x86::Instruction) -> String {
        ...
        if size == 80 {
            format!(
                "runtime::fpu::read_f80(&ctx.memory, {})",
                self.gen_addr(instr)
            )
        } else if size != 64 {
            format!(
    ...
    fn fpu_set_mem(&self, instr: &iced_x86::Instruction, expr: String) -> String {
        ...
        let size = mem_size(instr);
        if size == 80 {
            return format!("runtime::fpu::write_f80(&mut ctx.memory, {addr}, {expr});");
        }
    ...
    pub fn codegen_fpu(&mut self, instr: &iced_x86::Instruction) -> bool {
        ...
        match instr.mnemonic() {
            ...
            Ficom | Ficomp => {
                let size = op_size(instr, 0);
                self.line(format!(
                    "ctx.cpu.fpu.set_cmp({}.partial_cmp(&({} as i{size} as f64)));",
                    self.fpu_get_reg(0),
                    self.get_op(instr, 0)
                ));
            ...
            Ftst => {
                self.line(format!(
                    "ctx.cpu.fpu.set_cmp({}.partial_cmp(&0.0));",
                    self.fpu_get_reg(0)
                ));
            }
            Fcompp | Fucompp => {
                self.line(format!(
                    "ctx.cpu.fpu.set_cmp({}.partial_cmp(&{}));",
                    self.fpu_get_reg(0),
                    self.fpu_get_reg(1)
                ));
                self.line("ctx.cpu.fpu.pop();");
