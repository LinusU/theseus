...
impl<'a> CodeGen<'a> {
    ...
    pub fn resolve_cont(&mut self, addr: u32, from: u32) -> String {
        if let Some(block) = self.blocks.get(&addr) {
            ...
            // Only log the first reference to a given unknown address; this keeps
            // the translation output readable when multiple branches target the
...
mod tests {
    ...
    fn codegen_handles_xchg() {
        ...
        codegen.gen_instr(&instr).unwrap();
        assert!(
            codegen.buf.contains("let (old_0, old_1) ="),
            "xchg should use a tuple swap: {}",
            codegen.buf
        );
        assert!(
            !codegen.buf.contains("let t ="),
            "xchg should not use a manual temp swap: {}",
            codegen.buf
        );
    }
    ...
    fn codegen_handles_packed_add_sub() {
        ...
        for (bytes, want) in [
            ...
            ([0x0f, 0xd4, 0xc1], "paddq"),
            ([0x0f, 0xdd, 0xc1], "paddusw"),
            ([0x0f, 0xf8, 0xc1], "psubb"),
            ([0x0f, 0xfa, 0xc1], "psubd"),
            ([0x0f, 0xfb, 0xc1], "psubq"),
            ([0x0f, 0xd9, 0xc1], "psubusw"),
    ...
    #[test]
    fn codegen_uses_segmented_far_branch32_targets() {
        // A `66`-prefixed far branch in a segmented (16-bit) module targets a
        // seg:ofs pair; the gatherer enqueues IP::Seg, so codegen must too.
        let state = crate::State::default();
        let mut codegen = super::CodeGen::new(&state, false);

        for (bytes, want) in [
            (
                // jmp far 001b:12345678
                &[0x66, 0xea, 0x78, 0x56, 0x34, 0x12, 0x1b, 0x00][..],
                "Cont(unk_5828)",
            ),
            (
                // call far 001b:12345678
                &[0x66, 0x9a, 0x78, 0x56, 0x34, 0x12, 0x1b, 0x00][..],
                "ctx.callf32(0x8, 0x1b, Cont(unk_5828))",
            ),
        ] {
            codegen.buf.clear();
            let mut decoder =
    ...
    fn codegen_handles_phsubsw() {
        ...
        for (bytes, want) in [
            // phsubsw mm0, mm1 (0F 38 07 C1)
            (
                &[0x0f, 0x38, 0x07, 0xc1][..],
                "ctx.cpu.mmx.mm0 = phsubsw(ctx.cpu.mmx.mm0, ctx.cpu.mmx.mm1);",
            ),
            // phsubsw xmm0, [eax] (66 0F 38 07 00)
            (
                &[0x66, 0x0f, 0x38, 0x07, 0x00],
                "phsubsw_xmm(ctx.cpu.xmm.xmm0, ctx.memory.read::<[u32; 4]>(ctx.cpu.regs.eax))",
            ),
        ] {
            codegen.buf.clear();
            let mut decoder =
